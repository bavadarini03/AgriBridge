...
const TransportFarmerModule = {
  ...
  async submitRequest(e) {
    ...
    try {
      const crop = document.getElementById('t-req-crop').value;
      const weight = parseFloat(document.getElementById('t-req-qty').value);
      const pickupLat = parseFloat(document.getElementById('t-req-pickup-lat').value);
      const pickupLng = parseFloat(document.getElementById('t-req-pickup-lng').value);
      const pickupAddr = document.getElementById('t-req-pickup-addr').value;
      const destLat = parseFloat(document.getElementById('t-req-dest-lat').value);
      const destLng = parseFloat(document.getElementById('t-req-dest-lng').value);
      const destAddr = document.getElementById('t-req-dest-addr').value;
      const notes = document.getElementById('t-req-notes').value.trim();

      // Haversine distance (km) between pickup and destination
      const R = 6371;
      const dLat = (destLat - pickupLat) * Math.PI / 180;
      const dLng = (destLng - pickupLng) * Math.PI / 180;
      ...
      // Earnings estimate
      const estCost = Math.max(80, Math.round(50 + 12 * distKm + 8 * (weight / 100)));

      // Get current Firebase user; fall back to transport_token (farmer logged in via transport portal)
      const currentUser = firebase.auth().currentUser;
      const farmerId = currentUser?.uid || localStorage.getItem('transport_token') || null;
      const farmerUser = JSON.parse(localStorage.getItem('transport_user') || 'null');
      const farmerName = farmerUser?.name || (typeof farmerProfile !== 'undefined' ? farmerProfile.name : 'Farmer');

      // Write directly to Firestore transportRequests
      ...
      const transportReq = {
        ...
        crop: crop,
        weight: weight,
        pickupLocation: { name: pickupAddr, latitude: pickupLat, longitude: pickupLng },
        dropLocation: { name: destAddr, latitude: destLat, longitude: destLng },
        estimatedDistance: distKm,
        estimatedTime: Math.max(10, Math.round(distKm * 3)),
      ...
      await db.collection('transportRequests').add(transportReq);
      ...
      document.getElementById('t-req-success-msg').innerHTML = `
        Your request is live — drivers will see it immediately.<br>
        Est. cost: ₹${estCost.toLocaleString('en-IN')} &nbsp;·&nbsp; Distance: ${distKm} km
      `;

      tToast('✅ Request submitted! Drivers can now see it.', 'success', 6000);
      this.loadRequests();
    } catch (err) {

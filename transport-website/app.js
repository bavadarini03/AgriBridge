...
async function loadDriverProfile() {
  ...
  if (driverData?.driver) {
    ...
    updateAvailabilityUI(driverData.driver.status);
  }
...
/* ---- Availability UI sync ---- */
function updateAvailabilityUI(status) {
  document.querySelectorAll('.avail-btn').forEach(btn => {
    btn.classList.toggle('active', btn.getAttribute('data-status') === status);
  });
  document.querySelectorAll('.avail-status-text').forEach(el => {
    el.textContent = fmtStatus(status);
    el.className = `avail-status-text status-${status}`;
  });
  if (AppState.driver) AppState.driver.status = status;
}
...
window.setAvailability = async function(status) {
  const prevStatus = AppState.driver?.status;
  try {
    updateAvailabilityUI(status);              // optimistic
    SocketManager.setAvailability(status);     // no-op shim
    await API.drivers.setAvailability(status); // writes to Firestore
    Toast.show(`Status set to ${fmtStatus(status)}`, 'success');
  } catch (err) {
    Toast.show(err.message, 'error');
    if (prevStatus) updateAvailabilityUI(prevStatus); // revert on failure
  }
};

/* ---- Global Socket (Firebase) events ---- */
function bindGlobalSocketEvents() {
  SocketManager.on('driver:availability_ack', (data) => {
    updateAvailabilityUI(data.status);
  }, 'global');

  SocketManager.on('driver:availability_changed', (data) => {
    if (data.driverId === AppState.driver?.id) {
      updateAvailabilityUI(data.status);
    }
  }, 'global');

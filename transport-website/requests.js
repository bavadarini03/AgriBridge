...
function renderRequestsList() {
  ...
  if (_reqsCurrentList.length === 0) {
    if (_reqsReason === 'no_vehicle') {
      list.innerHTML = `
        <div class="empty-state">
          <div class="empty-icon">🚛</div>
          <h3 class="empty-title">No Vehicle Registered</h3>
          <p class="empty-message">Please register a vehicle to receive transport requests.</p>
          <button class="btn btn-primary mt-2" onclick="navigateTo('vehicle')">Register Vehicle</button>
        </div>`;
      return;
    }
    if (_reqsReason === 'zero_capacity') {
      list.innerHTML = `
        <div class="empty-state">
          <div class="empty-icon">⚠️</div>
          <h3 class="empty-title">Vehicle Capacity Not Set</h3>
          <p class="empty-message">Your registered vehicle has a capacity of <strong>0 kg</strong>. Update your vehicle capacity so eligible transport requests can be matched to you.</p>
          <button class="btn btn-primary mt-2" onclick="navigateTo('vehicle')">Update Vehicle Capacity</button>
        </div>`;
      return;
    }
    if (_reqsReason === 'no_driver_profile') {
      list.innerHTML = `
        <div class="empty-state">
          <div class="empty-icon">👤</div>
          <h3 class="empty-title">Driver Profile Not Found</h3>
          <p class="empty-message">Your driver profile could not be found in the system. This may happen if registration was incomplete. Please try refreshing — your profile will be auto-recovered.</p>
          <button class="btn btn-primary mt-2" onclick="loadRequests()">🔄 Retry</button>
        </div>`;
      return;
    }
    list.innerHTML = `
